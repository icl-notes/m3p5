...
(Exercise: why is $ \br{T\br{t}, N\br{t}, B\br{t}} $ orthonormal for all $ t $ equivalent to
...
Similarly, we define the \textbf{differential} of a smooth function $ f : S \to \R $ if $ v \in T_pS $ satisfies $ v = \alpha'\br{0} $ for $ \alpha = \br{-\epsilon, \epsilon} \to S $ as
...
Let $ N : S \to S^2 $. What is $ dN_p : T_pS \to T_{N\br{p}}S^2 $? $ T_{N\br{p}}S^2 $ is all vectors orthogonal to $ N\br{p} $, which is $ T_pS $. We use this identification to write $ dN_p : T_pS \to T_pS $.
